...
public interface Facade {
  ...
  /**
   * Searches for results that match the specified query.<p>The parameter <code>termValuePairs</code> may contain
   ...
   *  </ul>
   *  <p>The values of these items must match exactly the values returned by the  methods getXValues(). Multiple
   * search can be implemented by separating the different values with &#37; (i.e. auto&#37;self will search for all
   * occurences of auto OR self for the specified key).<br>
   * For full text search one can add a key to <code>termValuePairs</code> with two doublequotes:</p>
   *  <ul>
   *    <li>&quot;&quot;</li>
   *  </ul>
   *  <p>The value belonging to this key may contain any text. (See  <a
   * href="http://lucene.apache.org/java/docs/queryparsersyntax.html" target="_blank">Lucene Query Syntax</a> for
   * further information on wildcards, fuzzy search, proximity search, boosting,...).</p>
   *  <p>The parameter <code>termValuePairs</code> may also contain the key
   *  <ul>
   *    <li>AllTermsRequired
   *  </ul>
   *  </p>
   *  <p>If the value is set to <code>false</code> the different terms will be combined by a logical OR, if set
   * to <code>true</code> the different terms are combinded by a logical AND.</p>
   *
  ...
  /**
   * Returns a sorted array of unique TargetEndUser values that are  stored in the repository.<p>The array
   * contains all sublevel values in a pathlike string representation (e.g. /a/b/c/d and /a/b/c and /a/b and /a)</p>
   *
  ...
  /**
   * Returns a sorted array of unique SchoolType values that are stored in the repository.<p>The array contains
   * all sublevel values in a pathlike string representation (e.g. /a/b/c/d and /a/b/c and /a/b and /a)</p>
   *
   * @return the sorted array
   *
   * @throws Exception
  ...
  /**
   * Returns a sorted array of unique SchoolDiscipline values that are stored in the repository.<p>The array
   * contains all sublevel values in a pathlike string representation (e.g. /a/b/c/d and /a/b/c and /a/b and /a)</p>
   *
   * @return the sorted array
   *
  ...
  /**
   * Returns a sorted array of unique SchoolDiscipline values that are related to the  provided schoolType value
   * and are stored in the repository.<p>The parameter <code>schooltype</code> should contain the full path, i.e.
   * someting like <code>/a/b/c</code></p>
   *  <p>The returned array contains all sublevel values in a pathlike string representation (e.g. /a/b/c/d and
   * /a/b/c and /a/b and /a)</p>
   *
  ...
  /**
   * Returns a sorted array of unique ProfessionalSituation values that are stored in the repository.<p>The array
   * contains all sublevel values in a pathlike string representation (e.g. /a/b/c/d and /a/b/c and /a/b and /a)</p>
   *
   * @return the sorted array
   *
