...
/**
 * @author Falco Paul, Open University Netherlands, OTO Meta/Z project Meta/Z
 *         application instance (implemented as a singleton object) This class
 *         offers "global" services to application modules The singleton pattern
 *         assures that only one instance will exist in the VM And yes, I know
...
public class MetaZ implements java.io.Serializable {
	...
	/**
	 * Global Meta/Z application instance, private and static to force singleton
	 * pattern
	 */
  ...
  private MetaZ() {
	 ...
	 * 
	 * @param clazz
	 *            classname where you would like a logger for
	 ...
	 * Works better than calling toString() as that will print actual content in
	 * a lot of cases...
	 * 
	 * @param object
	...
	/**
	 * Returns A path string (in location file system notation) for the given
	 * file object
	 * 
	 * @return A canonical path string (in location file system notation)
	 *         representing the given file object, or null if the file object is
	 *         not representable in the file system
	 */
	...
	/**
	 * Returns the Meta/Z root directory as definied by the user, or "/" if not
	 * set
	 * 
	 ...
	 * 
	 * @return A file object representing the Meta/Z root directory Returns null
	 *         if the MetaZ root directory canonical path could not be
	 *         constructed
	 */
	public File getRoot() {
		...
		try {
			...
			logger
					.warn("Could not construct a canonical path for the MetaZ root directory <"
							+ getRootSpec() + ">");
			return null;
		}
	...
	/**
	 * Returns A String representing the complete specification for the given
	 * relative file name
	 * 
	 * @return A String representing the complete path for the relative file
	 *         specification This is _NOT_ a verified path
	 */
	...
	/**
	 * Returns A File object representing the given file name (file name is
	 * relative to the root)
	 * 
	 * @return A File object representing the given file name (file name is
	 *         relative to the root) Returns null if the file canonical path
	...
	public File getRelativeFile(String relativeFileName) {
		...
		try {
			...
			logger
					.warn("Could not construct a canonical path for the relative file <"
	 ...
	 *         Returns null if the MetaZ configuration file directory path could
	 *         not be constructed
	 */
	...
	private Properties readProperties() {
		...
		try {
			...
			if (verboseOutput)
				logger
						.info("Could not open properties resource file from the user home <"
								+ fileName + ">");
		}
		...
		if (propStream == null) {
			...
			try {
				...
				if (verboseOutput)
					logger
							.info("Could not open properties resource file from the current directory <"
									+ fileName + ">");
			}
		...
		if (propStream == null) {
			...
			if (propStream == null)
				logger
						.warn("Could not open properties resource file from anywhere in the classpath location <"
		...
		if (propStream != null) {
			...
			try {
				...
				logger.warn("Exception <" + e.toString()
						+ "> occurred while opening properties from <"
						+ propStream.toString() + ">");
	...
	/**
	 * Looks up a resource named 'name' in the classpath, and return a stream
	 * pointing to that resource. The name is assumed to be absolute and MUST
	 * use "/" to separate package segments (with an optional leading "/"). The
	 * following names thus refer to the same resource:
	 * 
	 ...
	 * 
	 * @param resourceName
	 *            a resource name somewhere available in the classpath [may not
	 *            be null]
	 * @param loader
	...
	/**
	 * A convenience overload of getProperties(String, ClassLoader) that uses
	 * the current thread's context classloader.
	 * 
	 * @param resourceName
	 *            The property file name
	 * @return A Properties object seeded with the properties from the supplied
	 *         file name
	 */

	public static InputStream getResourceStream(String resourceName) {
		return getResourceStream(resourceName, Thread.currentThread()
	 ...
	 * 
	 * @param loader
	 *            classloader for which to get the classpath
	...
	/**
	 * A convenience overload of getClassPath(ClassLoader) that uses the current
	...
	/**
	 * Returns the classpath as a platform specific string Simply call
	 * getClassPathString(getClassPath()) to get the current (stringified )
	 * classpath
	 * 
	 * @param classPath
	 *            an array of URL's representing a class path
