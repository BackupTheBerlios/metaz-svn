...
/**
 * A stemmer for Dutch words. The algorithm is an implementation of the <a
 * href="http://snowball.tartarus.org/algorithms/dutch/stemmer.html">dutch stemming</a> algorithm in Martin Porter's
 * snowball project.
 *
...
public class DutchStemmer {
  ...
  /**
   * <p>A d-suffix, or derivational suffix, enables a new word, often with a different grammatical category, or
   * with a different sense, to be built from another word. Whether a d-suffix can be attached is discovered not from
   * the rules of grammar, but by referring to a dictionary. So in English, ness can be added to certain adjectives to
   * form corresponding nouns (littleness, kindness, foolishness ...) but not to all adjectives (not for example, to
   * big, cruel, wise ...) d-suffixes can be used to change meaning, often in rather exotic ways.</p>
  ...
  /**
   * undouble vowel If the words ends CVD, where C is a non-vowel, D is a non-vowel other than I, and V is double
   * a, e, o or u, remove one of the vowels from V (for example, maan -> man, brood -> brod).
   *
   * @param sb String being stemmed
   */
  ...
  private void unDouble(StringBuffer sb, int endIndex) {
    ...
    if (s.endsWith("kk") || s.endsWith("tt") || s.endsWith("dd") || s.endsWith("nn") || s.endsWith("mm") ||
        s.endsWith("ff")) {
