...
public class HttpCGI implements HttpProcessor, Runnable {
  ...
  protected void extractScriptName () {
    ...
    cgiScript = new File (HTTP.SERVER_LOCATION,
                          HTTP.translateFilename (scriptName.substring (1)));
  }
  ...
  protected void initEnv (InetAddress ip) {
    ...
    environment.addElement ("SERVER_PROTOCOL=" + "HTTP/" +
                            in.getVersion ());
    ...
    while (headerNames.hasMoreElements ()) {
      ...
      environment.addElement ("HTTP_" + name.toUpperCase ()
                              .replace ('-', '_') + "=" + in.getHeader (name));
    }
  ...
  public void processRequest (HttpOutputStream out) throws IOException {
    ...
    try {
      ...
      HttpException httpEx = new HttpException
        (HTTP.STATUS_INTERNAL_ERROR, "<PRE>" + trace + "</PRE>");
