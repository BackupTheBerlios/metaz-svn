...
public class SearchServiceImplAltTest extends TestCase
{
     ...
     */
    public SearchServiceImplAltTest(String sTestName)
    {
        super(sTestName);
    ...
    public static void main(String[] args)
    {
        String[] args2 = 
            {
                "-noloading", "org.metaz.repository.SearchServiceImplTest"
            };
     ...
     * Initialises three record instances and places them into the search index.
     *
     ...
     */
     public void setUp() throws Exception
     {
         ...
         Record rec1 = new Record(title1, secured1, fileFormat1,
         ...
         */
         ...
         uri2.setValue(
             "http://cgi.omroep.nl/cgi-bin/streams?/nps/raymannislaat/bb.20051223_tante.wmv");

         Record rec2 = new Record(title2, secured2, fileFormat2,
                 didacticFunction2, productType2, uri2);

         //optional metadata subject
         TextMetaData subject2 = new TextMetaData();
         subject2.setValue(
             "Het Beste van Raymann is Laat - gepresenteerd door Jörgen Raymann ");
         rec2.setSubject(subject2);

         //optional metadata description
         HtmlTextMetaData description2 = new HtmlTextMetaData();
         description2.setValue(
             "<p class=MsoNormal><span style='font-size:6.5pt;font-family:Verdana;color:black'>Tante\n" +
         ...
         keywords2.setValue(
             "muziek, rap, video, uitzending, tv, battel, battle, thebattle, raymann is laat, Jurgen, comedian, young, urban, tante Esselien");
         ...
         */
         ...
         didSc.setValue(
             "de leerlingen bekijken de video en lossen nadien vragen op");
         ...
         Record rec3 = new Record(title3, secured3, fileFormat3,
                 didacticFunction3, productType3, uri3);
    ...
    /**
     * Tests the method DoSearch2()
     * requires a hashmap as parameter
     *
    ...
    /**
     * Tests the method DoSearch()
     * requires a search string as parameter
     *
     * @throws Exception when search can not be completed
     * */
     ...
     {
         assertEquals(2,
             ((List) ssi.doSearch(MetaData.PRODUCTTYPE + FIELDDELIMITER +
                 "Document")).size());
         assertEquals(1, ((List) ssi.doSearch("nederlands")).size());
         assertEquals(1,
             ((List) ssi.doSearch(MetaData.SCHOOLDISCIPLINE + FIELDDELIMITER +
                 "Nederlands")).size());
         assertEquals(1,
         ...
         assertEquals(1,
             ((List) ssi.doSearch("nederlands" + WHITESPACE +
                 ...
                 WHITESPACE + MetaData.SCHOOLTYPE + FIELDDELIMITER +
                 "\"Voortgezet onderwijs\"")).size());
         assertEquals(1, ((List) ssi.doSearch("Buisonjé")).size());
         assertEquals(2,
             ((List) ssi.doSearch(MetaData.DIDACTICFUNCTION + FIELDDELIMITER +
                 "Leestekst" + WHITESPACE + MetaData.PRODUCTTYPE +
                 FIELDDELIMITER + "Document" + WHITESPACE +
         ...
         assertEquals(3,
             ((List) ssi.doSearch("monarchie OR uitzending OR nederlands.nl")).size()); //boolean
    ...
    public void testPresentResults(){
        ...
        for(int i=0; i<results.size();i++){
            System.out.println("RECORD " + Integer.toString(i+1));
            Record record = (Record) results.get(i).getObject();
            TextMetaData title = record.getTitle();
            if(title!=null){
                System.out.println("titel: "+title.getValue());
            }
            HtmlTextMetaData description = record.getDescription();
            if(description!=null){
                System.out.println("omschrijving: "+description.getValue());
            }
            TextMetaData keywords = record.getKeywords();
            if(keywords!=null){
                System.out.println("sleutelwoorden: "+keywords.getValue());
            }
            TextMetaData productType = record.getProductType();
            if(productType!=null){
                System.out.println("producttype: "+productType.getValue());
            }
            BooleanMetaData secured = record.getSecured();
            if(secured!=null){
                System.out.println("beveiligd: "+secured.getValue());
            }
            TextMetaData fileFormat = record.getFileFormat();
            if(fileFormat!=null){
                System.out.println("bestandsformaat: "+fileFormat.getValue());
            }
            HyperlinkMetaData uri = record.getURI();
            if(uri!=null){
    ...
    public void testGetDistinctValues(){
         ...
         String[] distinct = ssi.getDistinctValues(MetaData.DIDACTICFUNCTION);
         for(int i=0; i<distinct.length; i++){
             System.out.println(distinct[i]);
         }
         assertNotNull(ssi.getDistinctValues(MetaData.PRODUCTTYPE));
         System.out.println("De mogelijke waarden voor producttype zijn:");
         distinct = ssi.getDistinctValues(MetaData.PRODUCTTYPE);
         for(int i=0; i<distinct.length; i++){
            System.out.println(distinct[i]);
         }
        assertNotNull(ssi.getDistinctValues(MetaData.COMPETENCE));
        System.out.println("De mogelijke waarden voor competentie zijn:");
        distinct = ssi.getDistinctValues(MetaData.COMPETENCE);
        for(int i=0; i<distinct.length; i++){
           System.out.println(distinct[i]);
        }
        assertNotNull(ssi.getDistinctValues(MetaData.TARGETENDUSER));
        System.out.println("De mogelijke waarden voor beoogde eindgebruiker zijn:");
        distinct = ssi.getDistinctValues(MetaData.TARGETENDUSER);
        for(int i=0; i<distinct.length; i++){
           System.out.println(distinct[i]);
        }
        assertNotNull(ssi.getDistinctValues(MetaData.SCHOOLDISCIPLINE));
        System.out.println("De mogelijke waarden voor vakleergebied zijn:");
        distinct = ssi.getDistinctValues(MetaData.SCHOOLDISCIPLINE);
        for(int i=0; i<distinct.length; i++){
           System.out.println(distinct[i]);
        }
        assertNotNull(ssi.getDistinctValues(MetaData.SCHOOLTYPE));
        System.out.println("De mogelijke waarden voor schooltype zijn:");
        distinct = ssi.getDistinctValues(MetaData.SCHOOLTYPE);
        for(int i=0; i<distinct.length; i++){
           System.out.println(distinct[i]);
        }
        assertNotNull(ssi.getDistinctValues(MetaData.PROFESSIONALSITUATION));
        System.out.println("De mogelijke waarden voor beroepssituatie zijn:");
